...
function Colors:GetColor(Branch, Name)
	if Branch == "Class" then
		if not Ping.db.profile.Colors[Branch][Name] then
	...
	if not Ping.db.profile.Colors[Branch][Name].a then
		Ping.db.profile.Colors[Branch][Name].a = 1
	end
	return Ping.db.profile.Colors[Branch][Name]
end

local LastSet

function Colors:SetColor(Branch, Name, c)
	if type(Ping.db.profile.Colors[Branch][Name]) ~= "table" then
		Ping.db.profile.Colors[Branch][Name] = {}
	end
	Ping.db.profile.Colors[Branch][Name].r = c.r
	Ping.db.profile.Colors[Branch][Name].g = c.g
	Ping.db.profile.Colors[Branch][Name].b = c.b

	Ping.db.profile.Colors[Branch][Name].a = c.a
...
function Colors:UpdateColor(Branch, Name)
	...
	-- instead of indexing a nil table.
	if not ItemsToUpdate[Branch] or not ItemsToUpdate[Branch][Name] then
		return
	end
...
function Colors:Debug()
	for k1, Branch in pairs(ItemsToUpdate) do
		for k2, Name in pairs(Branch) do
			Ping:Print(getn(Name).." "..k1.." "..k2)

			local Items = ItemsToUpdate[k1][k2]

			for k, v in pairs(TypeToUpdate[k1][k2]) do
				if v == TYPE_TEXTURE then
					Ping:Print("Texture:" .. getn(Items[k]))
				elseif v == TYPE_BORDER then
					Ping:Print("Border:" .. getn(Items[k]))
				elseif v == TYPE_BACKGROUND then
					Ping:Print("Background:" .. getn(Items[k]))
				elseif v == TYPE_FUNC then
					Ping:Print("Func:" .. getn(Items[k]))
				elseif v == TYPE_FONT then
					Ping:Print("Font:" .. getn(Items[k]))
				end
			end
		end
	end
