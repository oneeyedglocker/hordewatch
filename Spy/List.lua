...
-- ============================================================
-- Healer detection (target-picker helpers)
-- ============================================================
...
--					print(spellId, " - ", spellName, " - ", class)
		...
		-- Confidence guard: a spell-derived level acts only as a floor. When
		-- we are NOT raising the level (level == nil) and we already hold a
		-- confirmed exact level for this player, keep it confident instead of
		-- letting the default isGuess = true re-stamp it as a guess (which
		-- would render e.g. a known "55" back to "55+").
		if level == nil and playerData and playerData.isGuess == false then
			isGuess = false
		end

		-- Confirmed-heal detection: if this enemy is the one CASTING a known
