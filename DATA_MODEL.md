...
7. **Automated time-of-day/day-of-week pattern prediction is now built too** (`lib/fingerprint.ts`, `components/ActivityFingerprint.tsx`, on the Trends page). Pick a player or guild and it buckets their historical sightings by hour-of-day and day-of-week, then answers "where are they most likely to be" for either right now or a picked day+hour. This is frequency-matching against history, explicitly **not** a trained model - `predictZone` falls back through three tiers (same day-of-week + hour window → any day at that hour → the subject's all-time top zone) as the matching window gets too sparse, and reports which tier won plus a sample-size caveat below `LOW_DATA_THRESHOLD` (8 total sightings) so the UI never states a thin guess with false confidence.
