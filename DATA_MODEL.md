...
4. **The Outland continent → pixel transform now accounts for HereBeDragons' coordinate split, but is still unverified against real play data.** A real import initially showed sightings clustered in one small spot instead of spread across zones - HereBeDragons' TBC Classic transform table (`HordeWatch/Libs/HereBeDragons/HereBeDragons-2.0.lua`, the `WoWBC` block) buckets most of Outland's raw coordinate space into `continentID` 0 or 1 with an offset added to `worldX`/`worldY`, and only a small leftover pocket keeps `continentID` 530 unmodified - the app was only accepting 530, so it was silently dropping most real sightings. Fixed: `ZoneMap.tsx` now accepts continentID 530/0/1, and `LeafletContinentMap.tsx` reverses the matching offset before applying wow.export's raw-coordinate transform (`OUTLAND_OFFSETS`). Verified with synthetic points that reconstruct to the same pixel regardless of which bucket they came from, but not yet checked against a real in-game sighting at a known location - worth a spot-check once you have one.
