...
1. **Real client data has already disproven one assumption this doc used to make confidently: TBC Classic's `mapID`s are not the commonly-cited "classic" numbers.** This repo initially shipped zone images under old textbook values (Hellfire Peninsula = 100, Netherstorm = 109, etc.) — a real import showed the actual `C_Map.GetBestMapForUnit()` values this client returns are 1944 and 1953 respectively, nowhere close. Fixed for those two (renamed to `1944.jpg`/`1953.jpg`), but the other 5 committed zone images (`102.jpg`, `104.jpg`, `105.jpg`, `106.jpg`, `108.jpg`) are now known to likely be filed under wrong mapIDs too, pending real sightings from those zones to confirm. **Lesson: don't trust "well-documented" WoW ID numbers without checking them against this specific client's actual output** — see `web/public/maps/README.md`.
2. **The Outland continent → pixel transform is confirmed wrong against real play data, and the exact cause isn't nailed down yet.** A real import (2 zones, Hellfire Peninsula and Netherstorm, both reporting `continentID` 530 — not 0 or 1, so the HereBeDragons offset-bucketing fix below wasn't even in play here) placed both sightings in empty space between the actual zone landmasses on the stitched continent image, confirmed by sampling the image's own pixels at the computed coordinates (transparent/off-terrain, not on either zone's rendered art). The transform's internal math checks out against synthetic self-consistency tests (exact corners/center reconstruct correctly, and points simulating each HereBeDragons bucket reconstruct to the same pixel) — but that only proves the code correctly implements the formula, not that the formula's assumption (that HereBeDragons' `worldX`/`worldY` for `continentID` 530 is directly usable against wow.export's raw ADT-tile corners) is actually correct. HereBeDragons reads from `UIMapAssignment.db2`, a different data source than wow.export's raw ADT/WDT tile grid; the two may not share one coordinate convention even where no explicit offset transform applies. **Do not trust the continent view's positions until this is resolved** — an in-app warning banner was added to the continent view, and the per-zone view (`mapX`/`mapY`, Blizzard's own native per-zone position, not implicated in this discrepancy) remains the reliable one meanwhile. Next step: get sighting data with a known real-world landmark (e.g. "this sighting was standing at Honor Hold") to solve for the actual transform empirically, rather than guessing again.
3. **`HereBeDragons-2.0` coordinate output is otherwise unverified against a live client** — built and syntax-checked by reading the library's documented API; item 2 above is the first concrete real-client discrepancy found.
...
5. **Only Outland has a continent map** — Eastern Kingdoms/Kalimdor would need their own continent image + coordinate-sidecar through the same mechanism (see "Zone map imagery" above) before the continent toggle covers them.
