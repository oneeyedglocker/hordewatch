...
14. **The time-range slider can now play back over time** (`lib/usePlayback.ts`, `components/PlaybackControls.tsx`) - hit play and a playhead sweeps across the selected [start, end] range (a few speed presets expressed as "sweep the whole range in ~12/30/60s", not a raw multiplier, since "20x" is meaningless without knowing the span), or drag the scrubber to any point manually. While playback is active, both map views narrow to a trailing window around the playhead (`trailWindowFor` - the larger of 15 minutes or 5% of the selected range) instead of showing the whole range at once, so old sightings age out and new ones appear as time sweeps forward - this reuses the *existing* per-player trail-drawing in `LeafletZoneMap.tsx`/`LeafletContinentMap.tsx` with zero changes to those components; feeding them a sliding window of points is all "watching movement" turned out to need. The presence list is scoped to the same trailing window while playing, so it always reflects who's actually near the playhead. Animation state updates every frame for smooth timing but only commits to React (and therefore rebuilds the map's markers) every ~150ms, regardless of dataset size, so a multi-thousand-sighting zone doesn't jank. A "Show full range" control exits playback and returns to the static all-at-once view; narrowing the outer range slider on its own never activates playback - only an explicit play or scrub does.
